			...
			on: function() {
				...
				if ( ! wpcom_newdash.statsJsLoaded ) {
					...
					var statsJs = [
						...
						'/wp-content/admin-plugins/blog-stats/blog-stats-async.js?20120925'
					];
...
var wpcom_newdash_actions = {
	init: function() {
		...
		// Publicize -- a.publicize
		$( 'div#tab-content' ).on( 'click', '#publicize_checked', function(e) {
			wpcom_newdash_actions.publicize_connect();
		});
		
		// Submit publicize blog selection box
		$('div#tab-content').on( 'click', '#publicize-box input[type=submit]', function(e) {
			e.preventDefault();
			wpcom_newdash_actions.publicize_connect();
		});
	...
	publicize_connect: function () {
		$( '.loading' ).spin( 'small' );
		var service = $('#publicize_service').val();
		var blog_id = $('#publicize_blog_id').val();
		var nonce   = $('#publicize').val();
		...
		this.request = $.get( '/wp-admin/admin-ajax.php', {
			...
			$('.publicize').replaceWith( response );
			$( '.loading' ).spin( false );
		});
	},
